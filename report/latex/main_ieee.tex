...
\title{Warehouse Layout Optimization: A Simulation Study of Storage
Assignment Policies at the Schneider Electric Manisa Plant Warehouse}

\author{\IEEEauthorblockN{Deniz Etensel, Deniz Ege Memeto\u{g}lu, Nehir Konyar,
Elif Bostanc\i, Yi\u{g}it Ka\c{c}ar, S\"umeyra Pul\c{c}a}
\IEEEauthorblockA{Department of Industrial Engineering\\
Izmir University of Economics, Izmir, T\"urkiye\\
Supervisor: Dr.\ Oktay Karaba\u{g}}}
...
\begin{IEEEkeywords}
discrete-event simulation, storage location assignment, warehouse slotting,
kitting, reach-truck utilization, common random numbers
\end{IEEEkeywords}
...
The warehouse at the Manisa plant operates in a dynamic environment where
pallet storage, kitting, and internal transport happen at the same time.
Several operational and capacity issues in the current structure limit its
scalability and efficiency.

The facility is a multi-level rack system built mainly for standard Euro
pallets ($80 \times 120$~cm), with a capacity of about 3{,}000 pallet
positions distributed across several modules, and each rack section holds
three pallets. The layout is organized into parallel aisles about three
metres wide, where handling is carried out mostly by reach trucks. This
narrow-aisle arrangement creates a strong dependency on shared equipment and
raises the risk of congestion. For safety, access to most standard racks is
restricted to one side, with the rear closed to prevent contact between
operators and vehicles. Frequently used materials are stored at ergonomically
accessible levels on dedicated fast-mover racks with double-sided access,
where replenishment is performed from the front by reach truck while
operators pick from the rear through the kitting aisle, so picking and
replenishment can proceed at the same time.

Materials enter through receiving and are assigned to storage based on stock
level and operational need. High-frequency items are generally placed on
fast-mover racks and less frequent items in upper levels. Retrieval for
production is done either through coordinated kitting or direct pallet
handling, and internal transport follows the fixed milkrun routes.

Despite this structured system, current storage and allocation policies do
not reflect actual material usage across the different production lines. The
existing classification uses an internal combined ABC/FMR tool, where the ABC
component represents material cost and the FMR component represents usage
frequency. Although the tool updates quarterly and flags materials that are
no longer or rarely used, it has a structural flaw: it treats all materials
as if they came from a single source and ignores the different quantities
each line consumes. The raw output is therefore not reliable for direct
operational use. As a workaround the plant analyses per-line kit consumption,
but the overall system still assigns locations by aggregate frequency without
considering line dependencies. This scatters the materials of one kit order
across different rack levels and zones~\cite{emde2012,limere2012}, which
raises coordination effort and delays material supply~\cite{limere2012}.

The layout also does not fully use the available space. Storage density could
be raised through layout redesign, extra racking, or better use of vertical
space, but such physical changes need quantitative assessment before they are
attempted. The plant currently follows internal rules that have not been
compared against alternatives under consistent conditions.

This study therefore targets the inefficiency that comes from the lack of a
structured, dependency-aware approach to storage assignment. Rather than
proposing immediate physical changes, it evaluates alternatives in a virtual
environment, which allows a low-cost assessment of policy changes before any
physical implementation. The alternatives range from established methods such
as Double ABC and time-based classification to a strategy tailored to the
plant, all measured against the current baseline.

\subsection{Motivation}
Warehouse efficiency is the main driver of production stability at this
plant. Material flow is governed by a milkrun system on 45-minute cycles that
completes nine tours per day to replenish the lines. Each station must hold
enough stock for one 45-minute interval plus a 20\% tolerance margin. Within
these limits, even small delays in retrieval or kit preparation can exhaust
the buffer and cause a line stoppage. Occupational health and safety is also
a design constraint: the project aims to raise throughput while keeping strict
separation between pedestrian operators and vehicles such as reach trucks and
forklifts.

The facility is operationally mature and has a culture of improvement. A
previous internal study removed one reach truck from the fleet without losing
performance, which shows that gains are achievable through better system
design rather than capital spending. The warehouse currently runs a fleet of
7 reach trucks, 3 forklifts, 7 milkrun trains, and 2 electric pallet trucks,
together with Kardex units for small parts.

Even so, inefficiencies persist. The main analytical weakness is the
classification tool, which treats all materials as one homogeneous group and
ignores product complexity and line destination. Operationally this creates
waiting waste during kit preparation: when the materials of one kit are
scattered, with some on accessible fast-mover racks and others in high
positions that need a reach truck, the operator in the kit corridor must wait
for an RT driver. This waiting extends lead time and reduces synchronization
with production.

The motivation of this study is therefore to provide a scalable, low-cost
solution through a data-driven slotting approach. By combining a
dependency-aware value-based (ABC) and frequency-based (FMR) analysis, the
work aims to place high-frequency materials in ergonomic, lower-level
positions so they can be picked by hand, removing the reach-truck dependency
from the kit path.
...
Simulation is widely used to evaluate warehouse performance and capacity.
Discrete-event simulation (DES) can represent stochastic arrivals, processing
times, and shared-resource constraints. Negahban and Smith~\cite{negahban2014}
show that simulation is effective for finding bottlenecks, assessing storage
utilization, and testing alternative configurations in manufacturing and
warehouse settings without disrupting real operations. Hapsari et
al.~\cite{hapsari2025} reach a similar conclusion for a pharmaceutical
warehouse, where a simulation surfaced bottlenecks that static ABC analysis
could not. A recurring limitation is that many studies focus on a single
aspect, such as routing or capacity, whereas a real warehouse needs an
integrated view that combines layout, routing, and operational decisions.

\subsection{Warehouse Layout Optimization}
Ardiansyah et al.~\cite{ardiansyah2024} study picking inefficiencies caused by
unoptimized layouts and irregular placement, and note that picking consumes
much more time than checking, packing, or loading. They combine Double ABC
analysis, which classifies inventory by both pick frequency and outer volume,
with Arena-based DES to model outbound processes. A one-way ANOVA confirmed a
significant reduction in moving time, showing that layout changes reduce
operator travel distance. Their work provides a validated framework for
optimizing limited-space warehouses, and it is the methodological basis for
the Double ABC policy evaluated here.
...
Warehouse design, storage location assignment, and order picking are often
studied separately, although in practice they are linked. Moghadam et
al.~\cite{moghadam2025} show that treating them independently leads to
suboptimal performance, because layout choices dictate where items are stored
and how efficiently they can be picked. The present study evaluates storage
and classification policies without altering the physical layout, an approach
supported by Amorim-Lopes et al.~\cite{amorim2021}, who show that combining
simulation with optimization can reduce travel distance and retrieval time
even when the layout is unchanged.

Decision support systems (DSS) are increasingly used in warehouse management
to support data-driven decisions. They help managers analyse operational data
and evaluate alternative scenarios for layout, material flow, and resource
use, replacing purely intuitive judgement. The literature indicates that DSS
approaches improve process visibility and decision quality, but they tend to
emphasize high-level decision frameworks rather than detailed modelling of the
underlying operations. Combining the DSS philosophy with simulation gives a
more complete solution, and that is the approach taken here: the operations
are modelled in detail first, which produces a validated, data-driven basis
that could later feed a broader decision-support framework.
...
kit preparation time~\cite{hanson2012,petersen2004}. These dependencies are
rarely addressed, and classification methods are usually applied uniformly
without adapting to such operational
characteristics~\cite{hausman1976,roodbergen2009}. The gap is therefore not a
shortage of assignment methods, but the failure to adapt and evaluate them in
a real warehouse that accounts for line-based consumption and kit-level
spatial dependencies.

The contribution of this study is a dependency-aware classification approach,
the proposed \policy{Line-aware Slotting} policy, and its evaluation against
five alternatives within a discrete-event simulation framework. The study
shows how established storage strategies can be adapted to real operating
conditions rather than to aggregate demand, and it provides a statistically
validated, application-oriented method~\cite{ardiansyah2024,kokroko2025} that
reduces waiting waste and improves material synchronization without physical
layout changes.
...
This study built a discrete-event simulation model of the Schneider Electric
Manisa warehouse from historical SAP data, layout information, BOM
relationships, and operational time studies, and used it to evaluate
alternative storage-assignment policies under realistic conditions without
disrupting the live operation.

Storage assignment has a clear effect on performance. Among the alternatives,
the proposed \policy{Line-aware Slotting} policy performed best: it reduced
mean kitting lead time from 12.37 to 6.91~minutes per order, about 44\%, and
lowered reach-truck utilization from 21.7\% to 1.3\%. One-way ANOVA and paired
$t$-tests confirmed that these improvements are statistically significant and
not the product of simulation noise.

The results show that the main source of inefficiency is not travel distance
but the dependency on reach-truck intervention during kit preparation. By
placing frequently used line-specific materials in hand-reachable positions,
the proposed policy reduces waiting, improves synchronization between the
warehouse and the production lines, and raises process efficiency. The broader
message is that a substantial gain is available from data-driven storage
assignment alone, without costly layout changes or automation.

\subsection{Managerial Implications}
The results offer practical guidance for managers who want to improve
efficiency with limited investment. \policy{Line-aware Slotting} can be
introduced gradually, by first relocating the most frequently used materials
to hand-reachable levels near the lines they serve. For a facility processing
about 400 kit orders per day, the lead-time reduction is a real opportunity to
cut waiting waste, improve resource use, and stabilize the process. The
simulation framework itself is a decision-support tool: alternative policies
can be evaluated virtually before any physical change, which lowers both risk
and cost.

\subsection{Limitations}
Several modelling choices and scope limits should be kept in mind when reading
the results.

\textbf{Extrapolation of operational times.} The timing parameters come from a
video time-motion study of 2{,}319 micro-events on the F400 line and were then
applied to the other eight product families. If F400 is not representative of
slower or faster lines, the lead-time estimates will shift. The sensitivity
analysis bounds this risk, but line-specific studies would refine the model.

\textbf{Validation data.} The model was fitted and validated against the same
SAP ZWM92 dataset, because complete historical KPI records were not available
for out-of-sample testing. The goodness-of-fit and daily-volume checks are
therefore internal-consistency tests rather than true holdout validation.

\textbf{SAP decoding and multi-bin splits.} Only 750 active materials had
fully decoded rack-bay-position codes, and 386 materials have multiple bins
that the model does not split between forward and reserve. The Actual SAP
baseline is therefore best read as an accurate SAP-plus-heuristic hybrid
rather than a pure bin-for-bin replay of the layout.

\textbf{Operator congestion.} Operator congestion in shared aisles is not
modelled; agents pass through each other freely. The model may therefore
slightly underestimate lead time under high-load policies such as the
travel-distance policy.

\textbf{Scope limited to order picking.} The scope is order picking and
kitting only. \policy{Line-aware Slotting} concentrates fast movers at low
levels, which will increase putaway and replenishment frequency for those
positions. The 1.3\% reach-truck utilization is a picking-only figure, so the
total reach-truck workload in a live deployment would be higher. Replenishment
scheduling, workforce planning, dynamic routing, and equipment allocation were
deliberately left out so that the effect of storage assignment could be
isolated.

\subsection{Future Work}
Several extensions are possible. Additional time studies on the other
production lines would provide line-specific parameters and improve accuracy.
Kit-based co-location could be combined with \policy{Line-aware Slotting} to
further reduce picking travel within a single order. A pilot in one warehouse
zone would allow a before-and-after comparison against real performance.
Finally, simulation could be combined with optimization methods such as
mathematical programming or metaheuristics to generate and evaluate storage
assignments automatically.

\subsection{Sustainable Development}
This study supports three of the United Nations Sustainable Development
Goals~\cite{un_sdg}. SDG~9 (Industry, Innovation, and Infrastructure) is the
most direct fit: replacing intuition-based assignment with a data-driven
simulation is evidence-based industrial practice at essentially no capital
cost, with no new equipment or expansion. SDG~12 (Responsible Consumption and
Production) applies because the proposed slotting makes better use of the
storage and handling equipment that already exists; lower reach-truck use
means less energy per kit, and avoiding building expansion conserves land and
materials. SDG~8 (Decent Work and Economic Growth) applies on the operator
side: moving high-frequency items to the manual band reduces the ergonomic
strain of mast-elevated retrieval and removes idle time spent waiting for a
truck. These are modest, honest improvements that follow directly from the
model's findings.
